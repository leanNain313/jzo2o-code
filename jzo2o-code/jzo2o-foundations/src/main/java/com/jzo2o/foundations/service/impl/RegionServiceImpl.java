...
public class RegionServiceImpl extends ServiceImpl<RegionMapper, Region> implements IRegionService {
     ...
     * @param id 区域id
     */
    @CacheEvict(value = RedisConstants.CacheName.JZ_CACHE,
            key = "'ACTIVE_REGIONS'")
    @Override
    public void active(Long id) {
        ...
        queryWrapper.eq("sale_status", FoundationStatusEnum.ENABLE.getStatus());
        List<Serve> serves = serveMapper.selectList(queryWrapper);
        if (CollUtil.isEmpty(serves)) {
            throw new ForbiddenOperationException("当前区域没有上架服务，无法上架");
        }
        //更新启用状态
    ...
    @Caching(
            evict = {
                    @CacheEvict(value = RedisConstants.CacheName.SERVE_ICON, key = "#id"), // 服务类型缓存
                    @CacheEvict(value = RedisConstants.CacheName.JZ_CACHE, key = "'ACTIVE_REGIONS'"), // 开通城市缓存
                    @CacheEvict(value = RedisConstants.CacheName.HOT_SERVE, key = "#id") // 热点数据缓存
            }
    )
    @Override
    public void deactivate(Long id) {
        ...
        queryWrapper.eq("sale_status", FoundationStatusEnum.ENABLE.getStatus());
        List<Serve> serves = serveMapper.selectList(queryWrapper);
        if (CollUtil.isNotEmpty(serves)) {
            throw new ForbiddenOperationException("当前区域还有上架服务，无法禁用");
        }
        //更新禁用状态
        LambdaUpdateWrapper<Region> updateWrapper = Wrappers.<Region>lambdaUpdate()
                .eq(Region::getId, id)
